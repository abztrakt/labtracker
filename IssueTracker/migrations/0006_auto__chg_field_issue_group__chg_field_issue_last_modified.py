...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        # Changing field 'Issue.group'
        db.alter_column('IssueTracker_issue', 'group_id', self.gf('django.db.models.fields.related.ForeignKey')(to=orm['LabtrackerCore.Group'], null=True))

        # Changing field 'Issue.last_modified'
        db.alter_column('IssueTracker_issue', 'last_modified', self.gf('django.db.models.fields.DateTimeField')(null=True))
    ...
    def backwards(self, orm):
        ...
        try:
            for i in orm.Issue.objects.all():
                try:
                    groups = orm['LabtrackerCore.Group'].objects.filter(items__icontains(i))
                    i.group = groups[0]
                except:
                    i.group = orm['LabtrackerCore.Group'].objects.get_or_create(name='Default',description='Default place for machines')
                i.save()
        except:
            pass
        # User chose to not deal with backwards NULL issues for 'Issue.group'
        #raise RuntimeError("Cannot reverse this migration. 'Issue.group' and its values cannot be restored.")

        # Changing field 'Issue.last_modified'
        db.alter_column('IssueTracker_issue', 'last_modified', self.gf('django.db.models.fields.DateTimeField')())
    ...
    models = {
        'IssueTracker.issue': {
            ...
            'group': ('django.db.models.fields.related.ForeignKey', [], {'to': "orm['LabtrackerCore.Group']", 'null': 'True', 'blank': 'True'}),
